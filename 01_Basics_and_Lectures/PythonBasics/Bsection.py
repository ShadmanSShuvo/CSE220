...
def stem_plot(ax, n, x, label, color = None):
...
def main():
    ...
    stem_plot(ax1, n, x, "original x[n]", color = "tab:blue")
    stem_plot(ax1, n, x_time, f"time shift by n0={n0}", color = "tab:red")
    stem_plot(ax1, n, x_phase_equiv, f"phase change by phi0={phi0_equiv:.3f}", color = "tab:green")
    ax1.legend()
    fig1.tight_layout()
   
    
    phi0 = 0.785  # an arbitrary phase change
    ...
    stem_plot(ax2, n, x_phase, f"phase change by phi0={phi0:.3f}", color = "tab:green")
    stem_plot(ax2, n, x_time_best, f"best time shift k={best_k}", color = "tab:blue")
    ax2.legend()
    fig2.tight_layout()
    plt.show()
