...
def time_scale_signal(x : np.ndarray, k : int) -> np.ndarray:
    ...
    m = n / k
    is_exact = (m == np.round(m))
    m_int = np.round(m).astype(int)
    ans = np.zeros_like(x)
    ans[is_exact] = x[m_int[is_exact] + INF]
    return ans
...
def time_scale_signal_interpolate(x : np.ndarray, k : int) -> np.ndarray:
    ...
    def sample(idx):
        ...
        vals[valid] = x[idx[valid] + INF]
        return vals

    x_lo, x_hi = sample(m_floor), sample(m_ceil)
    return np.where(m_floor == m_ceil, x_lo, (x_lo + x_hi) / 2)
