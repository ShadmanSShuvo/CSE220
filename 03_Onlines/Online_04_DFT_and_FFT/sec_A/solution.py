"""
Solution for Online 04 DFT and FFT - Section A1, A2
Constructs a hybrid image in the frequency domain.
"""
...
def hybrid_image(low_image, high_image, kernel, engine):
    ...
    if low_image.ndim == 3 and low_image.shape[2] == 3:
        ...
        planes = [
            hybrid_plane(low_image[:, :, c], high_image[:, :, c], kernel, engine)
            for c in range(3)
        ]
