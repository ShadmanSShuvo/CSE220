"""
Solution for Online 04 DFT and FFT - Section C1, C2
...
def embed_secret(cover, secret, strength, engine):
    """
    Hide the secret image in selected frequency components of the cover image.
    ...
    # TODO 1: Compute Fourier transform of both images
    cover_spectrum = transform_2d(cover_padded, engine)
    secret_spectrum = transform_2d(secret_padded, engine)

    # High frequency mask
    high_frequency_mask = create_frequency_mask(shape, cutoff_ratio=0.20)

    # TODO 2: Frequency domain embedding
    # S[K] = (1 - a)C[K] + aM[K]
    # S[!K] = C[!K]
    stego_spectrum = cover_spectrum.copy()
    stego_spectrum[high_frequency_mask] = (
        (1.0 - strength) * cover_spectrum[high_frequency_mask]
        + strength * secret_spectrum[high_frequency_mask]
    )

    # TODO 3: Inverse transform
    stego_image = inverse_2d(stego_spectrum, engine).real
...
def run(cover_path, secret_path, strength, engine_name, out_dir):
    ...
    cover = load_image(cover_path, as_gray=True)
    secret = load_image(secret_path, as_gray=True)

    if secret.shape[0] > cover.shape[0] or secret.shape[1] > cover.shape[1]:
    ...
    (
        ...
        secret_spectrum,
        stego_spectrum,
    ) = embed_secret(cover, secret, strength, engine)

    os.makedirs(out_dir, exist_ok=True)
    save_image(normalize_image(cover), os.path.join(out_dir, "cover.png"))
    save_image(normalize_image(secret), os.path.join(out_dir, "secret.png"))
    save_image(stego_image, os.path.join(out_dir, "stego.png"))

    save_comparison(
        [cover, secret, stego_image],
    ...
    difference = np.abs(stego_image - cover_normalized)
    mean_difference = float(np.mean(difference))
    maximum_difference = float(np.max(difference))

    print("embedding strength:", strength)
    print("mean cover/stego difference:", mean_difference)
    print("maximum cover/stego difference:", maximum_difference)
...
def main():
    parser = argparse.ArgumentParser(description="Frequency-domain image steganography")
    parser.add_argument("--cover-image", default="images/sunset512.png")
    parser.add_argument("--secret-image", default="images/skyline512.png")
    parser.add_argument("--strength", type=float, default=0.1)
    parser.add_argument("--engine", choices=["dft", "fft", "arbitrary"], default="fft")
    parser.add_argument("--out-dir", default="outputs/lab_steganography_student")
