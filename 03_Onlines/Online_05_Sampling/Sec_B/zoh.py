...
Predicts sinc droop and measures it from a synthesized staircase signal.
Run with:  python3 solution.py  or  python3 zoh.py
"""
...
def zoh_gain(f, fs):
    ...
    gain = |sinc(f / fs)|
    Note: np.sinc(x) is sin(pi*x) / (pi*x), which corresponds exactly to normalized sinc.
    """
